...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //Variables for function run_query
  std::string tempString = sql.c_str();                       //text sql
  std::string newSQL;                                         //santized sql
  std::smatch match;                                          //pattern match
  std::regex numInjection(" or [1-9]+=[1-9]+;");              //pattern match for "or value=value"
  std::regex wordInjection(" or '[a-zA-Z]+'='[a-zA-Z]+';");   //regex used for ease of ranges


  //search for any number matches for SQLi
  //if found, replace SQLi with semicolon
  //rerun after with newSQL
  if (std::regex_search(tempString, match, numInjection)) {
      std::cout << "\nInjection detected by number match";
      newSQL = std::regex_replace(tempString, numInjection, ";");
      return run_query(db, newSQL, records);
  }
  //search for any word matches for SQLi
  else if (std::regex_search(sql, match, wordInjection)) {
      std::cout << "\nInjection detected by word match";
