// cs405_M1_Trembley.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    //for each numeric type, a conditional statement to check for overflow
    //if overflow occurs, returns -1
    ...
    if (typeid(T) == typeid(char)) {
        ...
        for (i = 0; i < steps; ++i) {
            //if result and increment is greater than max, overflow has occured
...
{
    //for each numeric type, a conditional statement to check for underflow
   //if underflow occurs, returns -1
    ...
    if (typeid(T) == typeid(char)) {
        ...
        for (i = 0; i < steps; ++i) {
            //if result minus decrement is less than min, underflow has occured
...
void test_overflow()
{
    ...
    if (result == -1) {                                   
        std::cout << "OVERFLOW ERROR HAS OCCURED" << std::endl; //if the was an error, print to screen
    }
    else {
        std::cout << +result << std::endl;  //if no error, print result
    }
}
...
void test_underflow()
{
    ...
    std::cout << +result << std::endl; //print the results

    std::cout << "\tSubtracting Numbers With Overflow (" << +start << ", " << +decrement << ", " << (steps + 1) << ") = ";
    result = subtract_numbers<T>(start, decrement, steps + 1);
    if (result == -1) {                                   
        std::cout << "UNDERFLOW ERROR HAS OCCURED" << std::endl; //if underflow occured, print error message
    }
    else {
        std::cout << +result << std::endl;  //if no underflow, print results
    }
}
