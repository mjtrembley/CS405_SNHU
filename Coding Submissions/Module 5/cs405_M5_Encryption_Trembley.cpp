...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
    ...
    { // TODO: student need to change the next line from output[i] = source[i]
      // transform each character based on an xor of the key modded constrained to key length using a mod
        ...
        output[i] = source[i] ^ key[i % key_length]; //encryption of source data
    }
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    //Take time_T and convert to data type tm so the styling of date is correct
    //found below code somewhat outdated from C++11 to current
    //https://en.cppreference.com/w/cpp/chrono/c/strftime
    std::time_t current_time = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
    char buf[100] = { 0 };
    std::strftime(buf, sizeof(buf), "%Y-%m-%d", std::localtime(&current_time));

    //output and save required styling of file
    std::ofstream outFS;
    outFS.open(filename);
    outFS << student_name << std::endl;
    outFS << buf << std::endl;
    outFS << key << std::endl;
    outFS << data << std::endl;
    outFS.close();
